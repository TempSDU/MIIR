...
items_list = {}
interactions_list = {}
with open('raw/meta_Toys_and_Games.json', 'r', encoding='utf-8') as f:
    content = f.readlines()
for line in content:
    line = eval(line)
    item_id = line['asin']
...
with open('raw/ratings_Toys_and_Games.csv', 'r', encoding='utf-8') as f:
    content = f.readlines()
for line in content:
...
with torch.no_grad():
    for i in range(int(np.ceil(len(titles)/batch_size))):
        ...
        title_embeddings.append(output.cpu().numpy())
...
with torch.no_grad():
    for i in range(int(np.ceil(len(descriptions)/batch_size))):
        ...
        attention_mask = tokens['attention_mask'].cuda()
        output = bert(input_ids, attention_mask)
        description_embeddings.append(output.cpu().numpy())
