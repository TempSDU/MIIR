...
items_list = {}
interactions_list = {}
with open('raw/meta_Sports_and_Outdoors.json', 'r', encoding='utf-8') as f:
    content = f.readlines()
for line in content:
    line = eval(line)
    item_id = line['asin']
...
with open('raw/ratings_Sports_and_Outdoors.csv', 'r', encoding='utf-8') as f:
    content = f.readlines()
for line in content:
...
for item_id in list(items_list.keys()):
    if item_id not in items_map:  # efficiency is too low
        items_list.pop(item_id)
'''
temp_items_list = {}
for item_id in items_map:
    temp_items_list[item_id] = items_list[item_id]
items_list = temp_items_list
num = 0
