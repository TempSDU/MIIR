...
def trainfunction(accelerator, model, dataloader, optimizer, n_gpu, K, epoch, save_path, log_every_n_steps, accumulate_grad_batches, gradient_clip_val):
    ...
    for _ in range(K):
        for batch in ddp_dataloader:
            ...
            if step%accumulate_grad_batches == 0 or step == len(ddp_dataloader)//batch_size:
                index, session_ids, session_side_information, session_masks = batch
                output = ddp_model(session_ids[0], session_side_information, session_masks[0])
                if n_gpu > 1:
                    loss = ddp_model.module.rec_loss(output, session_ids[1], session_masks[1])
                else:
                    loss = ddp_model.rec_loss(output, session_ids[1], session_masks[1])
    ...
    ddp_model.cpu()
    accelerator.save(ddp_model.state_dict(), save_path+'train-'+str(epoch)+'.pth')
    all_acc_loss = accelerator.gather((acc_loss/acc_num).unsqueeze(-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pretrained_model', type=str, default=None, help='the path of the pretrained model')
    ...
    parser.add_argument('-mask_prob', type=float, default=0.5, help='the probability of an item being masked')
    ...
    if accelerator.is_main_process:
        ...
        if args.resume:
            fw = open(args.save_path+'train_result.txt', 'a')
        else:
            fw = open(args.save_path+'train_result.txt', 'w')
    accelerator.wait_for_everyone()
    ...
    if args.resume:
        ...
        model.load_state_dict(torch.load(args.save_path+'train-'+str(last_epoch)+'.pth'))
    else:
        model = MIIR(dataset_files['dataset_token'])
        if args.pretrained_model:
            accelerator.print('Load the pretrained model')
    ...
    dataset = TrainDataset_MLM(dataset_files['interactions_filepath'], dataset_files['categories_filepath'], dataset_files['brands_filepath'], dataset_files['titles_filepath'], dataset_files['descriptions_filepath'], dataset_files['missings_filepath'], dataset_files['train_rows_filepath'], args.max_interactions, args.mask_prob, dataset_files['masks_filepath'])
    ...
    while epoch < args.epochs:
        start = time.time()
        result = {}
        loss = trainfunction(accelerator, model, dataloader, optimizer, args.n_gpu, args.K, epoch+1, args.save_path, args.log_every_n_steps, args.accumulate_grad_batches, args.gradient_clip_val)
