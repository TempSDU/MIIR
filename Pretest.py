...
from DataLoader import TestDataset_MLM
from Utility import init_seeds, xavier_init, evaluatefunction
from MIIR import *


def pretestfunction(accelerator, model, dataloader, epoch, rank, save_path):
    results = []
    indices = []
    ...
    with open(save_path+'pretest-'+str(epoch)+'-'+str(rank)+'.txt', 'w') as f:
        i = 0
...
if __name__ == '__main__':
    ...
    next_epoch = 1
    if args.resume:
        with open(args.save_path+'pretest_result.txt', 'r') as f:
            content = f.readlines()
    ...
    epoch = next_epoch
    while epoch <= epochs:
        model.load_state_dict(torch.load(args.save_path+'pretrain-'+str(epoch)+'.pth'))
        start = time.time()
        pretestfunction(accelerator, model, dataloader, epoch, accelerator.process_index, args.save_path)
        if accelerator.is_main_process:  # merge results from all processes
            results = {}
            for rank in range(args.n_gpu):
                with open(args.save_path+'pretest-'+str(epoch)+'-'+str(rank)+'.txt', 'r') as f:
                    content = f.readlines()
                for line in content:
                    line = eval(line.strip())
                    index = line['index']
                    results[index] = line
                os.remove(args.save_path+'pretest-'+str(epoch)+'-'+str(rank)+'.txt')
            assert len(results) == len(dataset), 'len(results) should be equal to len(dataset)'
            average_result = {}
            with open(args.save_path+'pretest-'+str(epoch)+'.txt', 'w') as f:
